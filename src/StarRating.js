...
export default function StarRating(
    ...
    const [rating, setRating] = useState(defaltRating);
    ...
    return (
        <div style={containerStyles} className={className}>
            <div style={starContainer}>
                {Array.from({ length: starNum }, (_, i) => <Star
                    onRating={() => handleRating(i + 1)}
                    onHoverin={() => setTempRating(i + 1)}
                    onHoverout={() => setTempRating(0)}
                    key={i}
                    full={tempRating ? tempRating >= 1 + i : rating >= i + 1}
                    size={size}
                    color={color}
            ...
            <p style={textStyle}>{
                messages.length === starNum ? messages[tempRating ? tempRating - 1 : rating - 1] :
                    tempRating || rating || ""}</p>
        </div>
    )
}
...
function Star({ onRating, full, onHoverin, onHoverout, size, color }) {
    ...
    >
        ...
        >
            ...
            1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 
